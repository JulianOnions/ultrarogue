...
int which_item, which_type;
{
    ...
    reg int wh;
    reg char ch, newitem, newtype, *pt;
    int msz, whc;
    WINDOW *thiswin;
    int tsize;
    int pres_item = 1;
...
teleport()
{
	...
	while (which != (short)ESCAPE 
		&& which != (short)LINEFEED
		...
		when 'l': c.x++;
		when '*':
		    msg("Use h,j,k,l to position cursor, then hit return.");
	    }
	    c.y = max(c.y, 1);
	    c.y = min(c.y, LINES - 3);
	    c.x = max(c.x, 1);
...
passwd()
{
	...
	return FALSE;
    *sp = '\0';
    return (strcmp(PASSWD, crypt(buf, PASSWDSEED)) == 0);
}
