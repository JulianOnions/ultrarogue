...
register bool drop;
{
	...
	when SCROLL:
	    if (obj->o_count == 1)
		 sprintf(prbuf, "A %sscroll ", blesscurse(obj->o_flags));
	    else
		 sprintf(prbuf, "%d %sscrolls ", 
        ...
        when POTION:
	    if (obj->o_count == 1)
		 sprintf(prbuf, "A %spotion ", blesscurse(obj->o_flags));
	    else
		 sprintf(prbuf, "%d %spotions ", 
			obj->o_count, blesscurse(obj->o_flags));
	    pb = &prbuf[strlen(prbuf)];
...
struct linked_list *item;
{
	...
	*op = *((struct object *) ldata(obj));
	...
	if (item == NULL)	/* if item wasn't sold */
	    ...
	    msg("You'll be sorry you ever sold that!");
